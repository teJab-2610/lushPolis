...
function Sidebar() {
    ...
    return (
        <>
            <h2 style={{fontFamily:'Times New Roman',color:"green"}}>Available Rooms</h2>
            ...
            <h2 style={{fontFamily:'Times New Roman',color:"green"}}>Members</h2>
            <div className="beel">
                <hr />
            {members.map((member) => (
                user._id!==member._id &&
                <ListGroup.Item key={member.id} style={{ cursor: "pointer" }} active={privateMemberMsg?._id == member?._id} onClick={() => handlePrivateMemberMsg(member)} disabled={member._id === user._id}>
                    <Row>
                        ...
                        <img src={dp} className="member-status-img" />
                        </Col>
                        <Col xs={9}>
                            {member.name}
